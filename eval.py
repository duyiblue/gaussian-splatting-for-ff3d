#!/usr/bin/env python3
"""
Simple evaluation script: checkpoint in, comparison image out.
"""

import sys
import os
import torch
import numpy as np
import matplotlib.pyplot as plt
from argparse import ArgumentParser, Namespace

from scene import Scene, GaussianModel
from gaussian_renderer import render
from arguments import ModelParams, PipelineParams

def main():
    if len(sys.argv) != 2:
        print("Usage: python eval.py <checkpoint.ply>")
        print("Example: python eval.py ./output/abc123/point_cloud/iteration_30000/point_cloud.ply")
        sys.exit(1)
    
    ply_path = sys.argv[1]
    data_path = "/orion/u/duyi/recon3d/gaussian/gaussian-splatting-for-ff3d/data/obj_000000"
    
    if not os.path.exists(ply_path):
        print(f"Error: {ply_path} does not exist")
        sys.exit(1)
    
    print(f"Loading checkpoint: {ply_path}")
    
    # Extract iteration from path (e.g., iteration_30000 -> 30000)
    iteration_dir = os.path.basename(os.path.dirname(ply_path))
    if iteration_dir.startswith("iteration_"):
        iteration = int(iteration_dir.split("_")[1])
    else:
        print(f"Warning: Could not extract iteration from {iteration_dir}, using -1")
        iteration = -1
    
    # Extract model path (should be the parent of point_cloud directory)
    model_path = os.path.dirname(os.path.dirname(os.path.dirname(ply_path)))
    
    print(f"Model path: {model_path}")
    print(f"Iteration: {iteration}")
    
    # Setup model parameters correctly
    model_args = Namespace(
        source_path=data_path,
        model_path=model_path,
        images="images", depths="", resolution=-1, white_background=False,
        train_test_exp=False, data_device="cuda", eval=True,
        geometry_only=True, sh_degree=0
    )
    
    # Create gaussians and scene - let Scene handle PLY loading
    gaussians = GaussianModel(0, geometry_only=True)
    scene = Scene(model_args, gaussians, load_iteration=iteration, shuffle=False)
    
    # Get test cameras
    test_cameras = scene.getTestCameras()
    print(f"Found {len(test_cameras)} test views")
    
    if len(test_cameras) == 0:
        print("Error: No test cameras found!")
        return
    
    # Setup rendering
    pipe_args = Namespace(
        convert_SHs_python=False,
        compute_cov3D_python=False, 
        debug=False,
        antialiasing=False
    )
    pipe = PipelineParams(ArgumentParser()).extract(pipe_args)
    bg_color = torch.tensor([0, 0, 0], dtype=torch.float32, device="cuda")
    
    n_views = min(3, len(test_cameras))
    fig, axes = plt.subplots(n_views, 4, figsize=(16, 4*n_views))
    if n_views == 1:
        axes = axes.reshape(1, -1)
    
    with torch.no_grad():
        for i in range(n_views):
            camera = test_cameras[i]
            print(f"Rendering view {i+1}/{n_views}: {camera.image_name}")
            
            # Render
            render_pkg = render(camera, gaussians, pipe, bg_color, geometry_only=True)
            alpha_pred = render_pkg["alpha"][0].cpu().numpy()
            depth_pred = render_pkg["depth"].cpu().numpy()
            
            # Handle depth dimensions
            if len(depth_pred.shape) == 3:
                depth_pred = depth_pred[0]  # Remove batch dimension if present
            
            # For FF3D format, camera.original_image contains the mask (grayscale in all channels)
            gt_image = camera.original_image.cpu().numpy()
            gt_mask = gt_image[0]  # First channel contains the mask
            
            # Get ground truth depth from invdepthmap
            gt_depth = None
            if camera.invdepthmap is not None:
                gt_invdepth = camera.invdepthmap[0].cpu().numpy()
                gt_depth = np.zeros_like(gt_invdepth)
                valid = gt_invdepth > 0
                gt_depth[valid] = 1.0 / gt_invdepth[valid]
            
            # Debug: print some statistics
            print(f"  GT mask range: [{gt_mask.min():.3f}, {gt_mask.max():.3f}]")
            print(f"  Pred alpha range: [{alpha_pred.min():.3f}, {alpha_pred.max():.3f}]")
            if gt_depth is not None:
                valid_gt_depth = gt_depth[gt_depth > 0]
                valid_pred_depth = depth_pred[depth_pred > 0]
                if len(valid_gt_depth) > 0:
                    print(f"  GT depth range: [{valid_gt_depth.min():.3f}, {valid_gt_depth.max():.3f}]")
                if len(valid_pred_depth) > 0:
                    print(f"  Pred depth range: [{valid_pred_depth.min():.3f}, {valid_pred_depth.max():.3f}]")
            
            # Plot comparisons
            axes[i, 0].imshow(gt_mask, cmap='gray', vmin=0, vmax=1)
            axes[i, 0].set_title(f'GT Mask (View {i})' if i == 0 else 'GT Mask')
            axes[i, 0].axis('off')
            
            axes[i, 1].imshow(alpha_pred, cmap='gray', vmin=0, vmax=1)
            axes[i, 1].set_title('Predicted Alpha' if i == 0 else '')
            axes[i, 1].axis('off')
            
            if gt_depth is not None:
                # Use same color scale for both depth images
                valid_depths = np.concatenate([gt_depth[gt_depth > 0].flatten(), 
                                             depth_pred[depth_pred > 0].flatten()])
                if len(valid_depths) > 0:
                    vmin, vmax = np.percentile(valid_depths, [5, 95])
                    axes[i, 2].imshow(gt_depth, cmap='viridis', vmin=vmin, vmax=vmax)
                    axes[i, 3].imshow(depth_pred, cmap='viridis', vmin=vmin, vmax=vmax)
                else:
                    axes[i, 2].imshow(gt_depth, cmap='viridis')
                    axes[i, 3].imshow(depth_pred, cmap='viridis')
                axes[i, 2].set_title('GT Depth' if i == 0 else '')
            else:
                axes[i, 2].text(0.5, 0.5, 'No GT Depth', ha='center', va='center', 
                               transform=axes[i, 2].transAxes)
                axes[i, 2].set_title('GT Depth' if i == 0 else '')
                axes[i, 3].imshow(depth_pred, cmap='viridis')
            axes[i, 2].axis('off')
            
            axes[i, 3].set_title('Predicted Depth' if i == 0 else '')
            axes[i, 3].axis('off')
    
    plt.tight_layout()
    
    # Save with more descriptive name
    ply_name = os.path.splitext(os.path.basename(ply_path))[0]
    iteration_name = os.path.basename(os.path.dirname(ply_path))
    model_name = os.path.basename(model_path)
    output_name = f"eval_{model_name}_{iteration_name}.png"
    plt.savefig(output_name, dpi=150, bbox_inches='tight')
    print(f"Saved comparison to: {output_name}")
    
    plt.close()

if __name__ == "__main__":
    main()